...
router.get("/github", (req, res) => {
  ...
  if (!clientId) {
    console.error(
      "GitHub authentication failed: GITHUB_CLIENT_ID is missing from environment variables.",
    );
    return res
      .status(500)
...
router.get("/github/callback", async (req, res) => {
  ...
  try {
    ...
    console.error(
      "Error during GitHub authentication callback:",
      error?.response?.data || error.message,
    );
    res
      .status(500)
      .json({ error: "Failed to complete GitHub authentication." });
  }
});
